...
parser.add_argument('--model', default='spidercnn_cls_xyz', help='Model name [default: model]')
...
# parser.add_argument('--normal', type=bool, default=True, help='Whether to use normal information')
    ...
    with tf.Graph().as_default():
            ...
            for i in range(NUM_GPUS):
                with tf.variable_scope(tf.get_variable_scope(), reuse=True):
                    with tf.device('/gpu:%d'%(i)), tf.name_scope('gpu_%d'%(i)) as scope:
                        ...
                        pred = MODEL.get_model(pc_batch,
                            is_training=is_training_pl, bn_decay=bn_decay, num_classes=NUM_CLASSES)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch == (MAX_EPOCH-1):
                ...
                log_string("Max acc model saved in epoch: %d" % maxAcc_epoch)
                log_string("Max acc is: %f" % eval_acc_max)
